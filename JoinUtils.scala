  ...
  def prepareLookupOperation(): Array[JoinOperation] = {
    ...
    operationEntity.getInSocketList().asScala.zipWithIndex.foreach {
      case (in, i) =>
        ...
        joinHelperArray(i) = JoinOperation(in.getFromComponentId, in.getInSocketId, dataFrame, keyFields.getFields, false, keyFields.isRecordRequired, outSocketId, null)
