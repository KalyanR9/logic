...
class InputFileParquetComponent(iFileParquetEntity: InputFileParquetEntity, iComponentsParams: BaseComponentParams)
  ...
  override def createComponent(): Map[String, DataFrame] = {
    ...
    try {
      ...
      val dfScehma = iComponentsParams.getSparkSession().read.parquet(path).schema

      if (isStrict) {
        if (dfScehma.size != schemaField.size) {
          SchemaUtils().compareSchema(schemaField.toList, dfScehma.toList)
        }
        else {
          if (isSafe) { // strict true safe false
            df = iComponentsParams.getSparkSession().read.parquet(path)
          }
          else { // strict true safe false
            //SchemaUtils().compareSchema(schemaField.toList, dfScehma.toList) // need to put logic for null if schema have diff datatypes
            df = iComponentsParams.getSparkSession().read.parquet(path)
            val changedTypedf = df.withColumn("age", col("age").cast(org.apache.spark.sql.types.StringType))
            print("======================================================"+(df("salary").cast(types.NullType)).getItem("salary"))
